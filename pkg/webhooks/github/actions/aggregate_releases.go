	...
	if openPR != nil {
		...
		if frozen {
			...
			_, _, err = r.client.GetV3AppClient().PullRequests.CreateComment(ctx, r.client.Organization(), r.repoName, *openPR.Number, &v3.PullRequestComment{
				Body: v3.String(fmt.Sprintf("Release `%v` in repository %q was rejected because release is currently frozen. Please re-issue the release hook once this branch was merged or unfrozen. /cc @%s",
					p.TagName,
					p.RepositoryName,
					p.Sender,
				)),
